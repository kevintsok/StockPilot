#!/usr/bin/env python3
"""
E2E Transformer v3 with BC Warm-Start

Key insight: BC's action_head already learned good trading patterns from 6-dim state.
We use BC's action_head as the final layer, but train the Transformer to produce
BC-compatible features.

Architecture:
1. Transformer encoder processes raw price sequence
2. Projects output to BC-state-like 6-dim representation
3. BC's pretrained action_head makes the final decision
4. BC weights are FROZEN during training (only projection is trainable)

This combines:
- Transformer: learns good sequence representations
- BC: provides proven trading decision logic
"""
import argparse
import random
...
class ContinuousE2EWithBCWarmup(nn.Module):
    """
    E2E with BC warm-start.

    The Transformer learns to produce features that BC's action_head can use.
    BC's action_head is FROZEN - we only train the projection from Transformer output.
    """

    def __init__(
        self,
        bc_pretrained_path: str,
        feature_dim: int = 25,
        seq_len: int = 60,
        d_model: int = 128,
        nhead: int = 4,
        ...
        bc_state_dim: int = 64,  # BC's feature dim before action_head
    ):
        super().__init__()
        self.max_position_pct = max_position_pct

        # Load pretrained BC
        self.bc = BCNetwork(input_dim=6, hidden_dim=128)
        bc_state = torch.load(bc_pretrained_path, map_location='cpu')
        self.bc.load_state_dict(bc_state)
        print(f"Loaded BC pretrained from {bc_pretrained_path}")

        # Freeze BC completely
        ...
        # Projection: Transformer output (128) -> BC-state-like (64)
        # This is trainable - learns to map Transformer features to BC's expected input
        self.to_bc_state = nn.Sequential(
            nn.Linear(d_model, bc_state_dim),
            nn.LayerNorm(bc_state_dim),
            nn.ReLU(),
        )
    ...
    def forward(self, price_seq: Tensor) -> Tensor:
        ...
        # Project to BC state dim
        bc_features = self.to_bc_state(x)  # (batch, 64)

        # BC's frozen action_head makes decision
        action = self.bc.action_head(bc_features)  # (batch, 1)
        action = torch.sigmoid(action) * self.max_position_pct
...
def train_e2e_v3_bc_warmup(
...
) -> ContinuousE2EWithBCWarmup:
    """Train E2E with BC warm-start."""
    random.seed(42)
    np.random.seed(42)
    ...
    print(f"\n{'='*60}")
    print("E2E v3 Training (BC Warm-Start)")
    print(f"seq_len={seq_len}, d_model={d_model}, layers={num_layers}")
    print(f"BC pretrained: {bc_pretrained_path}")
    print(f"{'='*60}")
    ...
    # Only train to_bc_state projection, freeze BC
...
def main():
    parser = argparse.ArgumentParser(description="Train E2E v3 with BC Warm-Start")
    ...
    parser.add_argument("--save-path", type=str, default="models/e2e_v3_bc_warmup.pt")
